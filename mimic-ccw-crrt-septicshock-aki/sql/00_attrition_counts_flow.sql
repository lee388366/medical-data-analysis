...
--
-- 依赖：101、201、301 已建。201 无 sepsis_time，从 mimiciv_derived.sepsis3 取感染时间。
-- ============================================================
...
-- 201 表无 sepsis_time，从 sepsis3 取：感染时间 ≤ t0
sepsis_before_t0 AS (
  SELECT DISTINCT t0.stay_id
  FROM t0
  JOIN mimiciv_derived.sepsis3 s ON s.stay_id = t0.stay_id
  WHERE s.sepsis3 IS TRUE
...
rrt_before_t0 AS (
  SELECT DISTINCT t0.stay_id
  FROM t0
  JOIN mimiciv_derived.rrt r ON r.stay_id = t0.stay_id
...
crrt_before_t0 AS (
  SELECT DISTINCT t0.stay_id
  FROM t0
  JOIN mimiciv_derived.crrt c ON c.stay_id = t0.stay_id
...
fail_sepsis_before_t0 AS (
  SELECT COUNT(*)::int AS n
  FROM t0
  LEFT JOIN sepsis_before_t0 s ON s.stay_id = t0.stay_id
...
fail_vaso_t0 AS (
  SELECT COUNT(*)::int AS n
  FROM t0
  LEFT JOIN vaso_t0 v ON v.stay_id = t0.stay_id
...
fail_pre_t0_rrt AS (
  SELECT COUNT(*)::int AS n
  FROM t0
  INNER JOIN any_rrt_before_t0 x ON x.stay_id = t0.stay_id
),
...
flow AS (
  ...
  UNION ALL
  SELECT 7, 'Reason (201->301): No vasopressor overlap in (t0-6h, t0]', (SELECT n FROM fail_vaso_t0)
  UNION ALL
  SELECT 8, 'Reason (201->301): RRT/CRRT before t0', (SELECT n FROM fail_pre_t0_rrt)
)
SELECT step_order, step_label, n_value FROM flow;

CREATE INDEX IF NOT EXISTS idx_attrition_step
  ON data_extract_crrt.attrition_counts_v1 (step_order);

ANALYZE data_extract_crrt.attrition_counts_v1;

-- 在 Navicat 显示（需先有 101、201、301）
SELECT * FROM data_extract_crrt.attrition_counts_v1 ORDER BY step_order;
