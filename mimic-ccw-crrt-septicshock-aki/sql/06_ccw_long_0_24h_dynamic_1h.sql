...
--
-- Contents:
--   hour_index 0..23, t_start, t_end, crrt_start_time,
--   time-varying: uo_mlkgph_last6h, map_last1h, vaso_flag_last1h, ne_eq_last1h,
--   mv_flag_last1h, lactate_last6h, ph_last6h, k_last6h, hco3_last6h, scr_last6h
--
-- 本地 MIMIC: urine_output(stay_id,charttime,urineoutput); norepinephrine_equivalent_dose(starttime,endtime 无 charttime).
--
-- Optimizations (postgres-patterns): Explicit cohort columns; LATERAL per slice; NE 用时间重叠; 索引建议 + Navicat.
-- ============================================================

CREATE SCHEMA IF NOT EXISTS data_extract_crrt;

-- 步骤 1：本脚本每行 7 次 LATERAL，无索引会极慢。下列索引 505 未建（505 未用尿量/血管活性药/机械通气），此处补全；若 505 已跑则 vitalsign/chemistry/bg/ne 可能已有，IF NOT EXISTS 会跳过。
...
LEFT JOIN LATERAL (
  SELECT
    CASE
      ...
      ELSE (SELECT COALESCE(SUM(u.urineoutput), 0)
...
LEFT JOIN LATERAL (
  SELECT MAX(ne.norepinephrine_equivalent_dose) AS ne_eq_last1h
  FROM mimiciv_derived.norepinephrine_equivalent_dose ne
  WHERE ne.stay_id = h.stay_id
    AND ne.starttime < h.t_end
    AND COALESCE(ne.endtime, h.t_end) > h.t_end - interval '1 hour'
...
LEFT JOIN LATERAL (
  SELECT
    ch.potassium AS k_last6h,
    ch.bicarbonate AS hco3_last6h,
    ch.creatinine AS scr_last6h
...
CREATE INDEX IF NOT EXISTS idx_ccw_long_subject
  ON data_extract_crrt.ccw_long_0_24h_1h_v1 (subject_id);

ANALYZE data_extract_crrt.ccw_long_0_24h_1h_v1;

-- 在 Navicat 显示（需先有 301；若 505 已建 derived 索引，本脚本会更快）
SELECT * FROM data_extract_crrt.ccw_long_0_24h_1h_v1 ORDER BY stay_id, hour_index LIMIT 500;
